...
export default function handler(options: any): Rule {
  const templateSource = apply(url('./files/image'), [
    applyTemplates({
      ...strings,
      ...options
    }),
  ...
  return chain([
    ...
    App.make(WordpressService).declareInServiceProvider('app/Providers/BlockServiceProvider.php', `\\App\\Blocks\\ImageBlock::class,`)
  ]);
}
